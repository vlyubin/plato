...
def write_speech(person_info: str, topic: str, supporting: bool = True):
    ...
    system_prompt = ""
    user_prompt = f"""You are {person_info}. You are particinpating in a debate competition and are writing a {speech_type_keyword} speech for the following statment: "{topic}". Where possible, reference your own background and qualifications. Come up with short speech (120 words or less).{rebuttal_prompt}
Speech:"""
...
def judge_speeches(topic: str, supporting_speech: str, rebuttal_speech: str):
    ...
    """
    system_prompt = ""
    user_prompt = f"""You are a judge in a debate competition. Given a topic, supporting and rebuttal speeches, grade each speech on a range from 1 to 10. Penalize speeches that too short or make weak arguments.
Topic: {topic}
...
Supporting speech score, rebuttal speech score (comma-separated):"""
    ...
    user_prompt = f"""You are a judge in a debate competition. Given a topic, supporting and rebuttal speeches, give a brief critique of both speeches. Penalize speeches that too short or make weak arguments.
Topic: {topic}
